...
> [!abstract]+ TL;DR
> - 파드는 k8s의 컴퓨팅 기본 단위로 가상 IP를 가지며 컨테이너를 감싸는 래퍼
> - 컨트롤러는 desired state와 current state의 차이를 자동 보정, 디플로이먼트가 대표적 파드 컨트롤러
> - YAML 매니페스트로 리소스를 정의하고 `kubectl apply`로 클러스터에 적용
