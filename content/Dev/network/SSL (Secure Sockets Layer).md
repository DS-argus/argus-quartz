...
> 비대칭을 데이터 암호화에 직접 안 쓰는 이유는 성능이다. 비대칭 연산은 대칭보다 수천 배 이상 느려서, 웹페이지 하나가 받는 수백 개 리소스를 전부 비대칭으로 처리하면 비현실적이다. 그래서 느린 비대칭은 세션 키 교환에 한 번만 쓰고 대량 데이터는 대칭이 맡는다. 각 방식의 약점을 서로의 강점으로 메우는 설계다.
