...
- 네트워크 드라이버(bridge, overlay 등)가 리눅스 네트워크 네임스페이스, 브리지(bridge), veth 페어(veth), iptables, ipvs 등 커널 기능을 네이티브하게 사용
    ...
    - enp0sX, eth0 등: 리눅스 호스트의 실제 물리 [[200. Inbox/네트워크/4. NIC와 케이블#NIC (Network Interface Controller) 호스트와 유무선 통신 매체를 연결하고 정보 변환을 담당하는 MAC 주소가 부여되는 네트워크 장비|NIC]] 이름
