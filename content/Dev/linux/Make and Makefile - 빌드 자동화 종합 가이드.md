...
---
> [!abstract]+ TL;DR
> - `make`는 수정 시각을 비교해 바뀐 부분만 빌드하는 자동화 도구, `Makefile`은 그 작업 명세서
> - 핵심 구조는 `target: prerequisites`와 탭 들여쓴 `recipe` 묶음, 본질은 의존성 그래프 기반 증분 빌드
> - 변수, 자동 변수, 패턴 규칙을 익히면 재사용 가능한 빌드 스크립트, 요즘은 태스크 러너로도 사용
