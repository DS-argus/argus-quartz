...
export function shouldSkipAutoPermalink(filePath) {
  return stripContentPrefix(toPosixPath(filePath)).toLowerCase() === "index.md"
}
...
export function ensurePermalinks(markdownFiles) {
  ...
  for (const filePath of markdownFiles.map(toPosixPath)) {
    if (!existsSync(filePath)) continue
    if (shouldSkipAutoPermalink(filePath)) continue
